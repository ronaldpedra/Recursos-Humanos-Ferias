...
class Usuario(db.Model, UserMixin):
    ...
    identidade = db.Column(db.String(20), nullable=False, unique=True)
...
class Secao(db.Model):
    ...
    chefe_id = db.Column(db.Integer, db.ForeignKey('usuario_id'))
...
class SolicitacaoFerias(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    solicitante_id = db.Column(db.Integer, db.ForeignKey('usuario_id'), nullable=False)
    periodo_aquisitivo_id = db.Column(db.Integer, db.ForeignKey('periodo_aquisitivo_id'), nullable=False)
