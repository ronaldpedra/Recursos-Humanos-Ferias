...
class PeriodoAquisitivo(db.Model):
    ...
    solicitacoes_vinculadas = db.relationship(
        'SolicitacaoFerias', back_populates='periodo_aquisitivo')
...
class SolicitacaoFerias(db.Model):
    ...
    periodo_aquisitivo_id = db.Column(db.Integer, db.ForeignKey(
        'periodo_aquisitivo.id'), nullable=False)

    data_inicio = db.Column(db.Date, nullable=False)
    data_fim = db.Column(db.Date, nullable=False)
    dias_solicitados = db.Column(db.Integer, nullable=False)

    # Ex: "30_DIAS", "15_DIAS", "10_DIAS", "DESCONTO"
    tipo_solicitacao = db.Column(db.String(50))
    status = db.Column(db.Enum(StatusFerias), nullable=False,
                       default=StatusFerias.SOLICITADA)
    ...
    periodo_aquisitivo = db.relationship(
        'PeriodoAquisitivo', back_populates='solicitacoes_vinculadas')
